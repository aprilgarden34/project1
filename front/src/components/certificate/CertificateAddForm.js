...
}) 
{
  ...
  // (확인) 버튼을 눌렀을 때 서버로 입력된 값이 추가되도록 작동하도록 하는 함수.
  ...
  const handleSubmit = async (e) => {
    ...
    // title, description, when_date의 value값으로 아래 폼에서 입력한 값이 들어감.
    // user_id에는 위에서 porp으로 가져온 portfolioOwnerId값이 들어감. 
    ...
    // certificate를 추가하는 과정이 끝났으므로, isAdding을 false로 바꾸어 편집화면 끔
    setIsAdding(false);
  };
